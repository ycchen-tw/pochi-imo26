...
if [[ -f "$FM_POCHI_ROOT/runtime/READY.json" ]]; then
  echo "Runtime already prepared: $FM_POCHI_ROOT/runtime"
  exit 0
fi
RUN_DIR="${RUN_DIR:-${ARC_RUNS:-/data/home/ycc/work/runs}/fm-pochi-setup-$(date -u +%Y%m%dT%H%M%SZ)}"
mkdir -p "$RUN_DIR"
python3 "$CODE_DIR/bootstrap_runtime.py" "$FM_POCHI_ROOT" "$RUN_DIR"
source "$CODE_DIR/env.sh"
"$FM_POCHI_ROOT/runtime/uv" pip install --python "$VENV/bin/python" -r "$REPO_DIR/evaluation/requirements.txt"
"$FM_POCHI_ROOT/runtime/uv" pip install --python "$VENV/bin/python" --no-deps --reinstall nvidia-cutlass-dsl-libs-cu13==4.5.2
bash "$REPO_DIR/sglang_patches/apply_patches.sh" "$VENV" \
  "$FM_POCHI_ROOT/runtime/proof-pilot/deploy/w4a8/humming_w4a8.py"
...
base = Path(os.environ['FM_POCHI_ROOT'])
venv = Path(os.environ['VENV'])
assert Path(sys.base_prefix).resolve() == (base/'runtime/pybase').resolve()
...
(base/'runtime/READY.json').write_text(json.dumps(report,indent=2)+'\n')
